...
export function drawScribe(ctx, w, h, t, age, state = 'idle') {
  ...
  // ── Quill nib (centered, pointing down)
  ...
  // ── Ink drop (teardrop at nib tip)
  const dropTipY = nibTipY + 5 * s
  let dropS, dropA
  if (state === 'idle') {
    dropS = 1 + 0.38 * Math.abs(Math.sin(t * Math.PI / 2.8))
    dropA = 0.9
  } else if (state === 'thinking') {
    dropS = 1 + Math.min(age, 1) * 0.8
    dropA = 0.85
  } else if (state === 'writing') {
    const cycle = t % 1.2
    dropS = 1 + 0.28 * Math.abs(Math.sin(cycle * Math.PI / 1.2))
    dropA = 0.9
  } else if (state === 'done') {
